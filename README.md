...
| 分析写入    | `src/analyze.ts`                           | 已完成                    |

### 模块结构

- `src/cli.ts`：CLI 入口，支持 `luagraph init`、`luagraph status` 和 `luagraph analyze` 命令。
- `src/lib.ts`：公共库出口。
...
| Analyze   | `submit/test-agent-parse.sh`     | `vp test test/analyze.test.ts && vp check`，18 文件解析后 Kuzu 有数据                       |
| Verify    | `submit/test-agent-verify.sh`    | CLI + `vp test test/verify.test.ts`，analyze 后 status 输出非零计数                    |
...
| systems-parse  | 已完成  | 解析符号写入 Kuzu           | parser 提取所有 class/function/method |
